...
{
	// JokerCrypt implementation
	if (length < 0x400)
		return;

	if ((flags & 0x204) == 0x204)
	{
		// XOR the length bytes - same as in decrypt, since XOR is self-reversible
		data[0x200] ^= (unit8)length;
		data[0x201] ^= (unit8)(length >> 8);
		data[0x202] ^= (unit8)(length >> 16);
		data[0x203] ^= (unit8)(length >> 24);

		// Check for special magic header used in the arithmetic coder
		if (0x718E958D == *(unit32 *)data)
		{
			unit8 input[0x200];
			memcpy(input, data, 0x200);
			int remaining = *(int *)(input + 8);
			int src = 12;
			int dst = 0;

			// Create frequency tables
			unit32 ranges_hi[0x100];
			unit32 ranges_lo[0x101] = {0};
			...
			unit32 denominator = ranges_lo[0x100];
			unit8 symbol_table[65536]; // Max possible value for denominator

			// Fill symbol lookup table
			for (int i = 0; i < 0x100; ++i)
			{
				unit32 low = ranges_lo[i];
				unit32 high = ranges_lo[i + 1];
				int count = (int)(high - low);
				for (int j = 0; j < count; ++j)
					symbol_table[low + j] = (unit8)i;
			}

			// Range encode - inverse of the decode operation
			unit32 low = 0;
			unit32 high = 0xFFFFFFFF;

			// Read initial value - big endian
			unit32 current = (input[src] << 24) | (input[src + 1] << 16) | (input[src + 2] << 8) | input[src + 3];
			src += 4;

			for (int i = 0; i < remaining; ++i)
			{
				unit8 symbol = data[dst++];

				// Update range based on symbol frequency
				unit32 range = high / denominator;
...
void extra_encrypt_shojomama(unit8 *data, unit32 length, unit32 flags)
{
	if (length < 0x400)
		return;
...
void extra_encrypt_makifes(unit8 *data, unit32 length, unit32 flags)
{
...
void ReadIndex(char *fname)
{
	...
	{
		// 初始化 flag1 和 flag2，可以从原始WAR文件中获取或使用默认值
		WARC_Header.flag1 = 0; // 使用默认值或从原始文件中读取
		WARC_Header.flag2 = 0; // 使用默认值或从原始文件中读取
		...
		// 读取目录中的所有文件
		FileNum = 0;
		while ((ent = readdir(dir)) != NULL && FileNum < 5000)
		{
			// 跳过 . 和 ..
			...
			// 初始化文件信息
			memset(WARC_Info[FileNum].name, 0, 32);
			// 直接使用文件名，不带前缀
			strncpy((char *)WARC_Info[FileNum].name, ent->d_name, 31);

			// 初始化其他字段，实际值会在 packFile 中设置
			WARC_Info[FileNum].offset = 0;
			WARC_Info[FileNum].comprlen = 0;
			WARC_Info[FileNum].uncomprlen = 0;
			GetSystemTimeAsFileTime(&WARC_Info[FileNum].time_stamp); // 使用当前时间

			// 使用默认标志，或者根据文件类型设置特定标志
			WARC_Info[FileNum].flags = 0x80000000; // 默认加密标志
...
void packFile(char *fname)
{
	...
	{
		// 直接使用文件名，不添加前缀
		sprintf(srcname, "%s", WARC_Info[i].name);
		dsize = MultiByteToWideChar(932, 0, srcname, strlen(srcname), NULL, 0);
		MultiByteToWideChar(932, 0, srcname, strlen(srcname), wsrcname, dsize);
		wsrcname[dsize] = L'\0';

		src = _wfopen(wsrcname, L"rb");
		if (src == NULL)
		{
			printf("无法打开文件 %s，跳过\n", srcname);
			continue;
		...
		if (WARC_Info[i].uncomprlen >= 8)
		{
			...
			if ((flag & 0xFF000000) != 0) // 沿用之前的定义，这里是永远加密的
			{
		...
		printf("%s offset:0x%X comprlen:0x%X uncomprlen:0x%X flags:0x%X\n",
			   WARC_Info[i].name, WARC_Info[i].offset, WARC_Info[i].comprlen,
			   WARC_Info[i].uncomprlen, WARC_Info[i].flags);
	}
	...
	WARC_Header.index_offset ^= 0xF182AD82; // ¤?¤у
