...
void extra_decrypt(unit8 *data, unit32 length, unit32 flags) // JokerCrypt
{
	if (length < 0x400)
		return;
	...
	if ((flags & 0x204) == 0x204)
	{
		if (0x718E958D == *(unit32 *)data)
		{
			...
			for (int i = 0; i < remaining; ++i)
			{
				...
				unit8 symbol = symbol_table[(current - low) / range];
				data[dst++] = symbol;
...
void extra_decrypt_shojomama(unit8 *data, unit32 length, unit32 flags)
{
	if (length < 0x400)
		return;
...
void extra_decrypt_makifes(unit8 *data, unit32 length, unit32 flags)
{
